...
const PaymentShipping: React.FC<PaymentAddressProps> = ({
  ...
  const originalDate = new Date(Date.now());
  const newFreeDate = new Date(
    originalDate.getTime() + 14 * 24 * 60 * 60 * 1000
  );
  const newFastDeliveryDate = new Date(
    originalDate.getTime() + 7 * 24 * 60 * 60 * 1000
  );
      ...
      <div className="px-10">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-between p-4 bg-gray-100 w-[300px] sm:w-[1000px] rounded-md h-15">
            ...
            <div>
              <span className="font-light">
                {formatDate(newFastDeliveryDate)}
              </span>
            </div>
          </div>
        </div>
