...
const PasswordChange = () => {
  ...
  const UserPasswordChangeValidation = z
    .object({
      currentPassword: z
        .string()
        .min(3, "Password must be at least 8 characters"),
      newPassword: z.string().min(3, "Password must be at least 8 characters"),
      confirmPassword: z
        .string()
        .min(3, "Password must be at least 8 characters"),
    })
  ...
  const onSubmit = async (
    values: z.infer<typeof UserPasswordChangeValidation>
  ) => {
    //  const user = {
    //    email:email,
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="flex-1 space-y-6 font-SFPro "
      >
        <CustomFormField
